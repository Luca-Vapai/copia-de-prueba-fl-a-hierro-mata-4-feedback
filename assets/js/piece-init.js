// piece-init.js — single-project site
// Bootstraps the piece review page. URL params: ?piece=<id>&v=<n>
// (no `project` param needed — the site IS the project).
...
(async function () {
  ...
  try {
    ...
    transcriptData = await loadTranscript(null, versionData.transcript_file);
  ...
  document.getElementById('piece-name-header').textContent =
    `${config.name} · ${piece.name}`;
  document.getElementById('piece-title').textContent =
    `${piece.name} — v${versionData.version}`;
  document.getElementById('piece-meta').textContent =
  ...
  document.getElementById('btn-mark-end').addEventListener('click', () => {
    if (rangeStart == null) {
      alert('First click "Mark range start" at the beginning of the range.'); return;
    }
    const rangeEnd = Player.getCurrentTime();
    if (rangeEnd <= rangeStart) {
      alert('Range end must be after the range start.'); return;
    }
    Player.pause();
  ...
  function priorityClass(p) { return 'tag-priority-' + p.toLowerCase().replace(/[^a-z]+/g, '-'); }
  ...
  function renderList(comments) {
    ...
    if (comments.length === 0) {
      ...
      listEl.innerHTML = comments.map(c => {
        ...
        const tcLabel = c.timecode_start == null ? 'GENERAL'
      ...
      listEl.querySelectorAll('.comment-item').forEach(item => {
        ...
        item.querySelectorAll('button[data-action]').forEach(btn => {
          btn.addEventListener('click', () => {
            ...
            } else if (action === 'edit') { FormUI.openEdit(id); }
            else if (action === 'delete' && confirm('Delete this comment?')) Comments.delete(id);
          });
        });
      });
    }
  ...
  function updateSubmitState() {
    submitBtn.disabled = !(reviewerInput.value.trim().length > 0 && Comments.count() > 0);
  }
  ...
  const reviewerKey = `feedback-loop-reviewer-${config.id}`;
  ...
  submitBtn.addEventListener('click', async () => {
    ...
    if (!endpoint || endpoint.includes('PENDING')) {
      alert('The submission endpoint is not configured. Contact the project owner.'); return;
    }
    ...
    try {
      await Submit.send({
        endpoint, project_id: config.id, piece_id: piece.id,
        version: versionData.version, reviewer_name: reviewerName,
